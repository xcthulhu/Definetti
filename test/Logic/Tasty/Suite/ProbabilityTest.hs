...
instance Arbitrary p => Arbitrary (Probability p) where
  arbitrary = sized sizedProbability where
...
findModel'
  :: ProbabilityInequality (Atom Char) -> Maybe (NonEmpty (AtomModel Char))
findModel' = findModel
...
probabilityTheoryQC = testGroup
  ...
  , testCase "Exists a model where (-1 * a < b)"
    $ let p = ((-1) :* Pr a) :< Pr b in (|= p) <$> findModel' p @?= Just True
  ]
 where
  [a, b]  = bound <$> ['a', 'b']
...
probabilityInequalitySemanticsQC = testGroup
  ...
  [ testProperty
      ...
      $ \f -> let m = findModel' f in fmap (|= f) m == fmap (const True) m
  ]
