...
module Logic.Propositional.Internal.Tseitin
...
instance (Ord p, MonadPlus m, ConstrainedModelSearch d p m) =>
         ConstrainedModelSearch d (Definitional p) m where
...
instance (Ord p, MonadPlus m, ConstrainedModelSearch d p m) =>
         ModelSearch d (Propositional p) m where
